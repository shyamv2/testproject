...
if(isset($_SESSION['id'])){
  ...
  if(isset($_POST['target_id']) AND is_numeric($_POST['target_id'])){
    ...
    $post['alternative_target_id'] =  mysqli_real_escape_string($con, $_POST['alternative_target_id']);
    $post['target'] = 'assignment';
    $assign = find_assignment($con, $post['target_id']);
    $class = find_class($con, $assign['class_id']);
    if(is_already_enrolled($con, $_SESSION['id'], $class['id'])){
      if(isset($_POST['post']) AND !empty(trim($_POST['post']))){
        ...
        $coin = array();
        $coin['edu_value'] = 0.05;
        $coin['origin'] = 'posted to assignment';
