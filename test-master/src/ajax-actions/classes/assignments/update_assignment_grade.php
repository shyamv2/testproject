		...
		if(isset($_POST['class_id']) AND is_numeric($_POST['class_id'])){
			if(isset($_POST['user_id']) AND is_numeric($_POST['user_id'])){
				if(isset($_POST['assignment_id']) AND is_numeric($_POST['assignment_id'])){
					...
					$grade['assignment_id'] = mysqli_real_escape_string($con, $_POST['assignment_id']);
					...
					if(user_class_permission($con, $_SESSION['id'], $grade['class_id'])){
						if(isset($_POST['grade'])){
							...
							//verify if exist
							$sql_verify_if_exists = "SELECT id FROM members_assignments_grades
							WHERE assignment_id = {$grade['assignment_id']} AND (user_id = {$grade['user_id']} AND class_id = {$grade['class_id']}) limit 1";
							...
							if(mysqli_num_rows($r_verify_if_exists) > 0){
								...
								//Update
								$sql_update = "UPDATE members_assignments_grades SET grade = '{$grade['grade']}' WHERE id = {$old['id']}";
								...
								//Create
								$sql_create = "INSERT INTO members_assignments_grades
								(assignment_id, class_id, user_id, grade)
								VALUES
								(
										{$grade['assignment_id']},
										{$grade['class_id']},
